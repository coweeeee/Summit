...
- Password reset — **the app half is built and merged** (PR #41; `app/forgot-password.tsx`, `components/ResetPasswordGate.tsx`, `lib/passwordReset.ts`), but the flow is **still broken end to end for a configuration reason, not a code one.** `summit://login` is NOT present in Supabase Auth → URL Configuration → **Redirect URLs**, so GoTrue discards the `redirectTo` and falls back to Site URL (the Vercel share host). The emailed link opens a browser there carrying a perfectly valid recovery fragment that never reaches the app.

  This masqueraded as a code bug through two fixes. PR #43 corrected a real defect (`createURL('/login')` → `createURL('login')`, i.e. `summit:///login` → `summit://login`) and changed nothing observable, because the allow-list never matched either form. A code comment asserting `summit://**` "covers it" was wrong and kept the search pointed at the app.

  **Verify the allow-list directly instead of believing any comment, including this one.** Probing `/verify` with a junk token still exercises redirect resolution — GoTrue echoes an allow-listed `redirect_to` and falls back to Site URL for an unlisted one. No email, no user data:

  ```bash
  curl -sSD - -o /dev/null "https://sigupaldsyuaomgllyvw.supabase.co/auth/v1/verify?token=INVALIDPROBE&type=recovery&redirect_to=summit%3A%2F%2Flogin" | grep -i '^location:'
  ```

  A `Location` of `summit://login#error=...` means allow-listed. A `Location` of `https://summit-api-server.vercel.app#error=...` means rejected — which is what it returns today. Control: a path such as `https://summit-api-server.vercel.app/DISCRIMINATOR` echoes back, proving `redirect_to` is honoured in general and that the `summit://` rejection is specific.

  **The fix is a dashboard change, not a commit:** add `summit://login` to Redirect URLs. Supabase's own guidance prefers an exact entry over a `**` globstar in production. Nothing in the repo can assert this, and `resetPasswordForEmail` returns 200 either way, so there is no client-side signal when it is missing.
